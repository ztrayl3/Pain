...
def work(eeg, queue):
    ...
    while stop < length:
        ...
        # average across epochs, channels, and frequency bands for a single max alpha amplitude value
        alpha = np.mean(np.average(np.average(psds, axis=0), axis=0))
        timeseries.append(alpha)
...
def main():
    ...
    for gender in data.keys():
        ...
        while not tasks_that_are_done.empty():
            ...
            fill.append([sub, gender, level[-1], "Alpha_Lat", max_time])
            fill.append([sub, gender, level[-1], "Alpha_Amp", alpha])

    header = ["ID", "Sex", "Stimulus", "Component", "Value"]
    output = pandas.DataFrame(data=fill, columns=header)
    output.to_csv("Stats/{}_freq_A.csv".format(condition))  # save to csv
    return True
