...
ggboxplot(ratings, x = "Sex", y = "Pain", 
  ...
  ggtitle("", subtitle = "Stimulus Intensity")
...
```{r}
pain.model <- lmer(Pain ~ Sex * Stim * Condition + (1|Subject), data = ratings, REML = TRUE)
anova(pain.model)
rand(pain.model)
```

Since we have a significant interaction, we will only analyze the interaction effect.

```{r, warning=FALSE}
testInteractions(pain.model)
```

The results suggest that there is no significant effect of sex, only a significant effect of stimulus level and subject. The significant interaction (Sex:Stimulus) is revealed to be significant only with the lowest stimulus in females vs the highest stimulus in men (i.e. the most extremes). To ensure validity, we will also check the assumptions for an ANCOVA.

1.  Normality (Shapiro-Wilk test), p < 0.05 is non normal, but visualization is a safer way to confirm
...
2.  Homogeneity of Variance (Levene's test), p < 0.05 is non-homogeneous, but if there is little to no group imbalance (between male/female *or* between high/med/low stimulus) this is acceptable.
...
3.  Sphericity: a significant interaction would indicate a violation.

```{r, indent = "     "}
mauchly.test(pain.model, x = ~ Sex * Stim * Condition + (1|Subject))
```
...
simple_model <- function(df, title) {
  ...
  erp.model <- lmer(Value ~ Stimulus * Condition + (1|ID), data = temp, REML = TRUE)  # don't include Sex
  post.hoc <- emmeans(erp.model, ~ Stimulus)

  post.hoc <- data.frame(pairs(emmeans(erp.model, ~ Stimulus * Condition), adjust = "tukey"))
...
```{r}
model <- function(df, title, interact) {
  temp <- subset(df, df$Component==title)
  ...
  if (interact) {
    post.hoc <- testInteractions(erp.model, pairwise = "Sex", across = "Condition")
  } 
  else{
    post.hoc <- data.frame(pairs(emmeans(erp.model, ~ Sex), adjust = "bonferroni"))
    post.hoc$Signif <- signif(post.hoc$p.value)
  }
  print("Post-Hoc testing (if needed)")
  print(post.hoc)
  ...
  qqPlot(R)
  hist(R)
  print("Normality (Shapiro-Wilk test), p of < 0.05 is a violation")
  print(shapiro.test(R))
  
  # 2.  Homogeneity of Variance (Levene's test)
  print("Homogeneity of Variance (Levene's test), p of < 0.05 is a violation")
  print(leveneTest(R ~ temp$Sex * temp$Stimulus))
  
  # 3.  Homogeneity of Regression Slopes
  print("Homogeneity of Regression Slopes (significant interactions indicate a violation)")
  print(Anova(aov(Value ~ Sex * Stimulus, data = temp), type = 3))
  
  # 4.  Assumption of linearity
  print("Assumption of linearity, relationship between pain level and stimulus intensity for each sex must be     linear")
  print("LINEAR")
  # ggplot(temp, aes(as.numeric(Stimulus), Value, colour = Sex)) +
  # scale_color_manual(values = c("#2274A5", "#CE7DA5")) + geom_point(size = 3) + 
  # geom_smooth(formula = y ~ x, method = "lm", aes(fill = Sex), se = FALSE, alpha = 0.1) +
  # theme(legend.position="top")
  
  # plot results
  y_label <- paste(gsub('_Amp', '', title), "Amplitude (μV)", sep=" ")
  facet_labels <- c(`1` = "Low",
                    `2` = "Medium",
                    `3` = "High")
  ggboxplot(temp, x = "Sex", y = "Value",
            color = "Sex", palette = c("#2274A5", "#CE7DA5"),
            ylab = y_label, xlab = "Sex") +
    facet_grid(. ~ Stimulus, labeller = as_labeller(facet_labels)) +
    theme(legend.position="none") +
    ggtitle("", subtitle = "Stimulus Intensity")
}
```
...
```{r}
model(erp, "N1_Amp", interact=FALSE)
```
...
```{r}
model(erp, "N2_Amp", interact=TRUE)
```
...
```{r}
model(erp, "P2_Amp", interact=TRUE)
```
...
```{r}
model(freq, "Gamma_Amp", interact=FALSE)
```

$~$

**Baseline ERP amplitude explained by Sex**

```{r}
model(erp, "Baseline_Amp", interact=TRUE)
```

$~$

**Baseline Gamma amplitude explained by Sex**

```{r}
model(freq, "Baseline_Amp", interact=FALSE)
```
