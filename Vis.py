...
def pretty_plot(EEG, title, ref, picks, ylim):
    ...
    evokeds = dict(low=epochs['Stimulus/S  1'].crop(tmin=0, tmax=0.5).average(),
                   med=epochs['Stimulus/S  2'].crop(tmin=0, tmax=0.5).average(),
                   high=epochs['Stimulus/S  3'].crop(tmin=0, tmax=0.5).average())
    mne.viz.plot_compare_evokeds(evokeds, picks=picks,
                                 title=gender.capitalize() + " " + title,
                                 ylim=dict(eeg=ylim),
                                 colors=colors[gender],
                                 show=False)[0].savefig("Figures/" + gender + title + ".png")
...
for gender in data.keys():
    ...
    # N2
    pretty_plot(epochs, "N2", "average", ["Cz"], [-5, 5])
    # P2
    pretty_plot(epochs, "P2", "average", ["Cz"], [-5, 5])
...
for gender in data.keys():
    ...
    all_epochs.append(data[gender].copy().shift_time(-1, relative=True))
...
pd.DataFrame({"Time": axis,
              "Gamma": timeseries}).to_csv("Figures/GammaTS.csv")  # to a csv file for use in R
